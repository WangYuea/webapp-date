...
data:{
  ...
  selectday(e){
    ...
    console.log(this.data.date[id][index])
    let temp=this.data.date[id][index]
    if (!this.data.date[id][index].selected){
      this.data.date[id][index].selected=1
    }else{
      this.data.date[id][index].selected = 0
    }
    this.setData({
      date:this.data.date
    })
